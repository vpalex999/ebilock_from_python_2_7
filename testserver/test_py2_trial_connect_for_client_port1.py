...
class EhaTestCaseClient1(TestCase):
    ...
    def setUp(self):
        ...
        from twisted.internet import reactor
        self.endpoint = reactor.listenTCP(server_port1, self.factory, interface="192.168.10.168")
    ...
    def test_trial_connect_for_client_port1(self):
        ...
        def callback(status):
            if self.assertTrue(status, "Delay 1.5 seconds connection to local port {} from EHA".format(server_port1)):
                # print("Connect to local port:{} from EHA: {}".format(server_port1, status))
                allure.attach("callback", "Connect to local port:{} from EHA: {}".format(server_port1, status))
