...
java -jar ebi-emulator-0.4.2-SNAPSHOT.jar emulator_config.json

java -jar file.jar [config.json]

How to run without console:
Windows:
javaw -jar ebi-emulator-0.4.2-SNAPSHOT.jar emulator_config.json

Linux:
java -jar ebi-emulator-0.4.2-SNAPSHOT.jar emulator_config.json > /dev/null &
...
Telegram Rules:
    ...
    SET_DATA_16Bytes		- Set 16 Bytes DATA for telegrams(max is 10(9))
...
Package Rules:
    ...
    SET_TM_CYCLE_AB_FA05	- Set telegram CYCLE A=0xFA B=0x05 for rule: SET_TM_CYCLE_AB_00FF
    SET_TM_CYCLE_AB_00FF	- Set telegram CYCLE A=0x00 B=0xFF
